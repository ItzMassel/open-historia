...
const WorldMap = ({ isGlobe = false }) => {
  ...
  const workerLabelNames = useMemo(() => {
    ...
    return buildPolityLabelNames({
      owners: [
        ...(customRegionMeta.records ?? []).map((record) => record?.owner),
        ...Object.values(regionOwnershipOverrides ?? {}),
      ],
      polityOverrides,
      resolveDisplayName: (raw, owner) => translateLabel(resolveCountryDisplayName(raw, owner)),
    });
