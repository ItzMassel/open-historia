...
const HeroIconStat = ({ href, title, icon, alt, count }) => (
  <a
    ...
    rel="noopener noreferrer"
    title={title}
    style={{ alignItems: "center", color: "#fff", display: "flex", flexDirection: "column", flexShrink: 0, gap: "0.5rem", textDecoration: "none" }}
  >
    <span
      style={{
        alignItems: "center",
...
// On desktop the like/comment stats sit flush with the map preview's own
// outer edges below — matched by giving both the SAME width (80%, the map
// frame's own PREVIEW_FRAME_STYLE) as siblings of the same parent, rather
  ...
  return (
    <div style={{ color: "#fff" }}>
      ...
      {isMobile ? (
        ...
        >
          {likeStat}
          <div style={{ alignItems: "center", display: "flex", gap: "clamp(1rem, 3vw, 1.8rem)" }}>
            {importButton}
            {githubButton}
          </div>
          {commentStat}
      ...
      {post.description && (
        <p style={{ color: "rgba(244,244,246,0.8)", fontSize: "clamp(0.9rem, 2.2vw, 1.05rem)", lineHeight: 1.6, margin: "clamp(1.6rem, 5vw, 2.8rem) auto 0", maxWidth: "40rem", textAlign: "center" }}>
          {post.description}
        </p>
      )}
