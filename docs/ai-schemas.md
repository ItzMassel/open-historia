...
A **refused demand** costs a fixed 10 Loyalty, charged once. A demand is negotiable text, so the refusal has to be *marked*: a one-on-one reply carries a hidden `REFUSED_DEMAND: <overlord> -> <puppet>` line (`src/runtime/diplomaticEnvelope.js`), taught in the per-reply instruction beside `DIPLOMATIC_MEMORY` and `REACTION` — by name, and only to a speaker party to a subordination whose other party is in the room (`refusalPairsFor`). It once sat at the tail of the leader's system prompt instead, and a live run showed an Overlord answer the player's flat refusal with "Belarus's refusal is noted" and never mark it: a model follows the output contract it is given at the moment it writes, and a group turn — which answers as structured actions, not text — sets `refusedOverlord` and `refusedPuppet` on the `send_message` that carries it (`chatActions.js`). Both name BOTH parties rather than taking the refuser from who spoke, so either side can mark it: an AI Puppet marks its own refusal, and an AI Overlord marks the reply answering the *player's*, whose typed message carries no envelope. The pair rides on the saved message and through the thread log (`chatThreads.js`), and the next jump collects it with `chargeRefusals` (`gameState.js`). Whether a refusal has been charged is recorded in **`world.chargedRefusals`**, by message id, and not on the message: every world writer re-reads before it writes, while chats are saved from whatever copy a panel holds, so a stamp on the message was erased by the next stale save and the refusal charged on every jump after. The jump is told the engine charges it, so it does not also lower Loyalty for the same refusal.
