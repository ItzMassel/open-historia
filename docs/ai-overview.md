...
| `src/Game/AI/catalystRewind.js` | **Taking back a beat of a catalyst scene.** Each beat keeps what the player was shown when they chose it, so the scene can return to any beat; and which scene is one in progress (`isSceneInProgress`). See [Catalyst mode](#catalyst-mode-a-scene-the-player-asks-for). |
...
## Catalyst mode: a scene the player asks for

A catalyst scene is a moment played out beat by beat — a summit, an ultimatum, a night in a bunker — then written into the record as one event. Scenes used to be offered by the time skip itself: the jump's schema carried a `catalyst` on every answer, into a save that no panel ever showed. Now **nothing of it exists until the player asks**: the jump's schema has no `catalyst` (an answer still carrying one, from an edited prompt passage or habit, has it dropped by `normalizeGameplayPayload` rather than refused), the actions reference and the breadth contract no longer mention it, and a scene begins only from **Catalyst mode** — the yellow first button of *Settings → Tools* (`GameUI/catalyst.jsx`).

In the mode the player says what scene they want (or leaves it to the simulation), plays the offered choices or writes their own move, takes a move back, and **ends the scene** (written into the record, one request) or **sets it aside** (nothing written, no request). `createCatalyst({ request })` binds the creation to the request (`[THE PLAYER'S REQUESTED SCENE — BINDING]`: exactly this moment, not widened, escalated, resolved in advance or dressed with invented backstory) or, with none, asks for the moment the current state makes ready (`[CHOOSING THE SCENE]`); it starts from the campaign as the player has seen it and refuses while a reveal is unfinished. The scene is stamped `origin: "player"`, `request`, `startedOn`. **Time stands still while one is in progress** (`isSceneInProgress`: the player's, or any with a beat played): `simulateTimelineJump` refuses, and the time panel's skip controls are replaced by a note that leads back to the scene. A scene an older skip proposed and nobody played is not one in progress — it blocks nothing, and a new scene replaces it. **A step that fails changes nothing**: the canned text a task falls back on is not a scene anyone asked for, so `createCatalyst`, `advanceActiveCatalyst` and the resolution throw the reason instead (`sceneStepFailed`) and the panel shows it. `endActiveCatalyst()` writes the scene as it stands — or, with no beat played, simply closes it; `setAsideActiveCatalyst()` closes it.

Costs, said beside the buttons: starting is one request, each move one more, ending one more; taking back and setting aside cost none. Proven offline in `.lab/probes/phase6-probe.mjs` (a requested scene, a skip refused while it runs, a beat played and taken back for no request, the end written into the record in one request, a scene left to the simulation and set aside, a failed start leaving nothing behind, and a skip answer still offering a scene taken in one request with no scene after it); the panel's two states are rendered in `.lab/probes/panel-smoke.mjs`.
...
|---------|-----------|-----------------------------|----------------------|
| `jumpForward` | `submit_jump_result` | `simulateTimelineJump({mode:"jump"})` | Advance to a target date; events + impacts → world state. No scene: those begin only in Catalyst mode. |
...
| `catalystCreation` | `submit_catalyst_creation` | `createCatalyst({ request })` | Open the scene the player asked for in Catalyst mode (or the one the moment makes ready). |
| `catalystExecutor` | `submit_catalyst_execution` | `advanceActiveCatalyst` (and `rewindActiveCatalyst` with a choice) | Play one move of the scene. |
| `catalystSummary` | `submit_catalyst_summary` | the scene resolving, or `endActiveCatalyst` | The scene written into the record as one event. |
...
Only a jump writes one. `selectLastJumpRecord` steps over Game Master, catalyst and pregame entries when it reads.
